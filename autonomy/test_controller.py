...
def isolated_repo():
    work=Path(tempfile.mkdtemp(prefix="kefayat-auto-test-")); (work/"autonomy").mkdir(); (work/"knowledge").mkdir()
    shutil.copy2(CONTROLLER,work/"autonomy"/"controller.py"); shutil.copy2(KB,work/"knowledge"/"competencies.json"); shutil.copy2(PLAN,work/"autonomy"/"mission-plan.json"); return work
def run_controller(repo):
    env=os.environ.copy(); env["PYTHONHASHSEED"]="0"; env["KEFAYAT_ROOT"]=str(repo)
    p=subprocess.run(["python3",str(repo/"autonomy"/"controller.py")],cwd=repo,text=True,capture_output=True,env=env,check=False)
    assert p.returncode==0,p.stderr; return json.loads(p.stdout.strip().splitlines()[-1])
...
def test_kb_shape_and_identity():
    d=json.loads(KB.read_text(encoding="utf-8")); assert isinstance(d,dict) and d.get("records") and isinstance(d.get("coverage"),list)
    ids=[r["id"] for r in d["records"]]; assert len(ids)==len(set(ids)); assert all(r["grade"] in {1,2,3,4} for r in d["records"]); assert all(r["subject"] in {"arabic","mathematics","islamic_education","nurturing"} for r in d["records"])
def test_progress_and_identity_behavior():
    repo=isolated_repo(); first=run_controller(repo); second=run_controller(repo); state=read_state(repo); expected=sha256(repo/"knowledge/competencies.json")
    assert first["action"]=="VALIDATE_PLAN",first; assert second["action"]=="VALIDATE_KB",second; assert "PLAN_VALIDATED" in state["completed_tasks"]; assert "KB_VALIDATED" in state["completed_tasks"]; assert state["artifact_identities"].get("competencies.json")==expected,state
