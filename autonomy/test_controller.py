...
def test_controller_uses_canonical_object_schema_behavior():
 repo=isolated_repo(); first=run_controller(repo); second=run_controller(repo); state=read_state(repo); assert first["action"]=="VALIDATE_PLAN"; assert second["action"]=="VALIDATE_KB"; assert "KB_VALIDATED" in state["completed_tasks"]; assert state["artifact_identities"]["competencies.json"]==hashlib.sha256(KB.read_bytes()).hexdigest()
...
def test_real_persistent_progress_across_restart():
 repo=isolated_repo(); first=run_controller(repo); second=run_controller(repo); state=read_state(repo); assert first["action"]=="VALIDATE_PLAN"; assert second["action"]=="VALIDATE_KB"; assert "PLAN_VALIDATED" in state["completed_tasks"] and "KB_VALIDATED" in state["completed_tasks"]
...
if __name__=="__main__":
 for fn in (test_kb_shape_and_identity,test_controller_uses_canonical_object_schema_behavior,test_controller_has_safe_stop_release_and_progress_watchdog,test_kb_sha256_is_stable,test_real_persistent_progress_across_restart,test_corrupt_state_fails_closed,test_stagnation_detection_ignores_timestamp_and_history_noise,test_release_gate_cannot_claim_assurance_without_evidence): fn()
